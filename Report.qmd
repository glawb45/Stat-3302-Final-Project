...
Although we may have previously thought that all of our covariates would have a significantly high effect on crop yield, we were somewhat mistaken. The quasi-poisson distribution does not yield any AIC values because the distribution doesn’t depend on the same likelihood function as other distributions do. Therefore, before realizing that the quasi-poisson model yielded better results to us than the poisson model, we were using AIC values from the poisson model as a way to judge the best ones.

We did find that based on our best models (which have been noted in our R chunks), arable land had the weakest relationship and effect on the yield — this somewhat makes sense relative to all the other covariates because we would intuitively imagine that year or region or crop helps give the best correlation to the yield.

We can, in fact, see in our year vs. yield plots by entity, that there is a positive correlation in the logistic regression fitted line for almost every entity. Annual change over time notably distinguishes a positive correlation between the two variables, and this was consistent throughout almost all entities except a couple.

Based on our best models, since we cannot use AIC to compare our models, we use the residual deviance as the next-best alternative to determine the best models. We find that our best entity model — with entity, barley and cocoa beans as covariates — was our best overall model to estimate crop yield, with a residual deviance of 9.0465. This fits perfectly with what we previously said about cocoa beans yielding the highest multiplicative-odds increase when doing individual modeling. Barley was also determined to be a good factor in modeling the highest change in yield — both of these crop covariates were in all of our best models.

The next-best model we had was our year model, which had a residual deviance of 9.4761; followed by our best crops model: 10.5148 residual deviance; and followed by our best arable land model: 10.5235 residual deviance.

Further deductions reveal that arable land, as we had concluded, was our worst predictor, followed by year. We can also deduce that no interaction effects is, a lot of the time, better than having interaction effects because this makes more parameters, increasing both the AIC and residual deviance.

Some factors that could also have an effect on the crop yield would be the weather of specific regions, which is something our data did not provide. In future studies it would be beneficial to get weather data to look at that covariate alongside the arable land and region of where the crops are being grown. We believe that would have a more significant impact on each specific crop’s yield. Based on our findings, we can say that while fertilizer can be beneficial to crop yield, the biggest factors when considering where to grow certain crops heavily depends on the region/entity and the types of crops produced.
