...
crops.anova <- glm(round(Yield) ~ wheat + maize + soybeans + potatoes + cassava + barley + cocoa_beans, data = merged_data, family = "quasipoisson")

air.anova <- glm(round(Yield) ~ arable_land + wheat + maize + potatoes + cassava + barley + cocoa_beans, data = merged_data, family = "quasipoisson")

year.anova <- glm(round(Yield) ~ as.factor(Year) + wheat + maize + soybeans + potatoes + cassava + barley + cocoa_beans, data = merged_data, family = "quasipoisson")

entity.anova <- glm(round(Yield) ~ Entity + barley + cocoa_beans, data = merged_data, family = "quasipoisson")
...
(air.mod <- summary(glm(round(yield) ~ arable_land -1, data = full_merged_crops, family = "quasipoisson")))

(wheat.mod <- summary(glm(round(yield) ~ wheat -1, data = full_merged_crops, family = "quasipoisson")))

(rice.mod <- summary(glm(round(yield) ~ rice -1, data = full_merged_crops, family = "quasipoisson")))

(maize.mod <- summary(glm(round(yield) ~ maize -1, data = full_merged_crops, family = "quasipoisson")))

(barley.mod <- summary(glm(round(yield) ~ barley -1, data = full_merged_crops, family = "quasipoisson")))

(soybean.mod <- summary(glm(round(yield) ~ soybeans -1, data = full_merged_crops, family = "quasipoisson")))

(potato.mod <- summary(glm(round(yield) ~ potatoes -1, data = full_merged_crops, family = "quasipoisson")))

(beans.mod <- summary(glm(round(yield) ~ beans -1, data = full_merged_crops, family = "quasipoisson")))

(peas.mod <- summary(glm(round(yield) ~ peas -1, data = full_merged_crops, family = "quasipoisson")))

(cassava.mod <- summary(glm(round(yield) ~ cassava -1, data = full_merged_crops, family = "quasipoisson")))

(cocoa.mod <- summary(glm(round(yield) ~ cocoa_beans -1, data = full_merged_crops, family = "quasipoisson")))

(bananas.mod <- summary(glm(round(yield) ~ bananas -1, data = full_merged_crops, family = "quasipoisson")))
...
summary(glm(round(yield) ~ Entity*as.factor(Year) - 1, data = full_merged_crops, family = "poisson"))
